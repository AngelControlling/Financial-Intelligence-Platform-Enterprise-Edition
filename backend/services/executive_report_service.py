...
class ExecutiveReportService:
    """
    Builds a dependency-free, print-ready Executive Financial Report.

    The report opens in a new browser tab and uses the browser's native
    ...
    def build_html(
        ...
        *,
        ...
        risks = self._list_items(
            brief.risks,
            empty_text="No material risks detected.",
            item_class="risk",
        )
        opportunities = self._list_items(
            brief.opportunities,
            empty_text="No material opportunities detected.",
            item_class="opportunity",
        )
        actions = self._list_items(
            brief.actions,
            empty_text="No open management actions.",
            item_class="action",
        )
...
.toolbar button.primary {{
    background: linear-gradient(90deg, var(--blue), #7457e8);
...
.header {{
    ...
    color: white;
    background:
radial-gradient(circle at 90% 10%, rgba(47,128,237,.28), transparent 34%),
...
<div class="meta">
    ...
    Actual vs {escape(brief.comparison_label)} · {escape(currency)}
</div>
...
<span>Confidential Management Information</span>
    ...
    def _list_items(
        ...
        *,
        ...
        return "".join(
            (
                f'<div class="item {escape(item_class)}">'
                f'{escape(item)}</div>'
            )
